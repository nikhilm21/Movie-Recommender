...
shinyServer(function(input, output) {
  ...
  output$plot <- renderPlot({
    df <- read.csv("imdb2.csv",sep = ',')
    colm <- as.numeric(input$colo)
    if(input$plotoption == "Histogram"){
      hist(df[, colm], freq = FALSE, xlab = input$xaxisname, ylab = input$yaxisname, main = input$title)
    } else if(input$plotoption == "BarPlot"){
      barplot(df[, colm], xlab = input$xaxisname, ylab = input$yaxisname, main = input$title)
    } else if(input$plotoption == "Scatter"){
      scatter.smooth(df[, colm], xlab = input$xaxisname, ylab = input$yaxisname, main = input$title)
    } else {
      pie(table(df[, colm]))
    }
  })
